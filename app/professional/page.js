...
export default function ScheduleMeet() {
    ...
    const handleSchedule = async () => {
        ...
        setLoading(false);
        if (res.ok) {
            setResponseMessage(`Meet scheduled successfully! Join link: ${data.meetLink}`);
        } else {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
            <div className="bg-white shadow-md rounded-lg p-6 w-full max-w-md">
                ...
                <button onClick={() => signOut()} className="bg-gray-500 text-white px-4 py-2 rounded w-full hover:bg-gray-600 transition duration-200">
                    Logout
                </button>
